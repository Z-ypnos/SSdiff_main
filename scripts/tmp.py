...
class PatchMergeModule(nn.Module):
    def __init__(self,
        ...
        print(f'path_size: {patch_size_list}')
        self.scale = scale
        self.device = device
        # net.eval()
        ...
        # self.hisi = hisi
        if patch_merge_step:
            self.forward = self.sample # partial(self.patch_merge_step, split_size=patch_size_list)
        self.split_size = patch_size_list
    ...
    def forward_chop(self, x_list, shave=12, **kwargs):
        ...
        for x, patch_size in zip(x_list, patch_size_list):
            ...
            x_unfold = F.unfold(x, (h_padsize, w_padsize), stride=(int(hshave / 2),
                                                                   wshave // 2)).permute(2, 0, 1).contiguous()
        ...
        y_hw_cut = self.forward(*[s.to(device) for s in x_hw_cut_list], **kwargs).cpu()
        ...
        y_h_cut = self.cut_h(x_h_cut_list, x_size_list,
                             h_cut_list, w_cut_list,
                             hw_padsize_list, hw_shave_list, scale,
                             batchsize,
                             **kwargs)
        x_h_cut_list.clear()
        torch.cuda.empty_cache()

        y_w_cut = self.cut_w(x_w_cut_list, x_size_list,
                             h_cut_list, w_cut_list,
                             hw_padsize_list,
                             hw_shave_list, scale,
                             batchsize,
        ...
        y_h_top = self.cut_h(x_h_top_list, x_size_list,
                             h_cut_list, w_cut_list,
                             hw_padsize_list,
                             hw_shave_list, scale, batchsize,
                             **kwargs)
        y_w_top = self.cut_w(x_w_top_list, x_size_list,
                             h_cut_list, w_cut_list,
                             hw_padsize_list,
                             hw_shave_list, scale, batchsize,
                             **kwargs)
        ...
        for i in range(x_range_list[0]):
            res = self.forward(*[s[i * batchsize:(i + 1) * batchsize, 0, ...].to(self.device) for s in x_unfold_list],
                               **kwargs)
            torch.cuda.empty_cache()
            y_unfold.append(res)
        ...
        x_size, h_cut, w_cut, (h_padsize, w_padsize), (hshave, wshave) = \
            x_size_list[0], h_cut_list[0], w_cut_list[0], hw_padsize_list[0], hw_shave_list[0]
        b, c, h, w = x_size
        for s_unfold, s_h_top, s_w_top, s_h_cut, s_w_cut, s_hw_cut in zip(y_unfold, y_h_top, y_w_top, y_h_cut,
                                                                          y_w_cut,
                                                                          y_hw_cut):
            ...
            divisor = F.fold(
                F.unfold(s_ones, (h_padsize * scale - hshave * scale, w_padsize * scale - wshave * scale),
                         stride=(int(hshave / 2 * scale), int(wshave / 2 * scale))),
                ((h - h_cut - hshave) * scale, (w - w_cut - wshave) * scale),
                (h_padsize * scale - hshave * scale, w_padsize * scale - wshave * scale),
                stride=(int(hshave / 2 * scale), int(wshave / 2 * scale)))
    ...
    def cut_h(self, x_h_cut_list, x_size_list, h_cut_list, w_cut_list, padsize_list, shave_list, scale, batchsize,
              **kwargs):
        x_h_cut_unfold_list = []
        x_range_list = []
        for idx, (_, c, h, w) in enumerate(x_size_list):
            x_h_cut_unfold = F.unfold(x_h_cut_list[idx], padsize_list[idx],
                                      stride=(int(shave_list[idx][0] / 2), int(shave_list[idx][1] / 2))).permute(2, 0,
                                                                                                                 1).contiguous()  # transpose(0, 2)
            x_h_cut_unfold = x_h_cut_unfold.view(x_h_cut_unfold.size(0), -1, c,
                                                 *padsize_list[idx])  # x_h_cut_unfold.size(0), -1, padsize, padsize
        ...
        y_h_cut_unfold = []
        for i in range(x_range_list[0]):
            res = self.forward(*[s[i * batchsize:(i + 1) * batchsize, 0, ...].to(self.device) for s in x_h_cut_unfold_list],
                               **kwargs)
    ...
    def cut_w(self, x_w_cut_list, x_size_list, h_cut_list, w_cut_list, padsize_list, shave_list, scale, batchsize,
              **kwargs):
        x_w_cut_unfold_list = []
        x_range_list = []
        # 1, 3, 792, 120 -> (30, 120), s=(7, 30) -> 109, 1, 3, 30, 120
        for idx, (_, c, h, w) in enumerate(x_size_list):
            x_w_cut_unfold = F.unfold(x_w_cut_list[idx], padsize_list[idx],
                                      stride=(int(shave_list[idx][0] / 2),
                                              int(shave_list[idx][1] / 2))).permute(2, 0, 1).contiguous()
        ...
        for i in range(x_range_list[0]):
            ...
            #                                                       ...], [1, 1, 1], dim=1)], **kwargs)
            res = self.forward(*[s[i * batchsize:(i + 1) * batchsize, 0, ...].to(self.device) for s in x_w_cut_unfold_list],
                               **kwargs)
            torch.cuda.empty_cache()
            y_w_cut_unfold.append(res)
